...
internal sealed class OpenXmlElementEqualityComparer
    : IEqualityComparer<OpenXmlElement> {
  /// <summary>
  ...
  internal
  OpenXmlElementEqualityComparer(OpenXmlElementEqualityOptions options) {
  ...
  public bool Equals(OpenXmlElement? x, OpenXmlElement? y) {
    ...
    if (!OpenXmlElementEqualityComparer.PrefixAndQNameEqual(x, y,
                                                            this.Options)) {
      return false;
    }
    ...
    if (x.HasChildren) {
      ...
      int e1 = 0, e2 = 0;
      while (OpenXmlElementEqualityComparer.MoveNextAndTrackCount(
          ref tChilds, ref oChilds, ref e1, ref e2)) {
    ...
    for (int i = 0; i < x.ParsedState.Attributes.Length; i++) {
      ...
      if ((tAttr.Value == null && oAttr.Value != null) ||
          (tAttr.Value != null && oAttr.Value == null)) {
        return false;
      }
  ...
  /// <summary>
  /// Handles checking of all options that changes the behaviour of equality
  /// based on options in <see cref="OpenXmlElementEqualityOptions"/>.
  /// </summary>
  private bool CheckAndEquateSpecialOptions(OpenXmlElement x,
                                            OpenXmlElement y) {
    if (!this.Options.RequireParsed) {
      if (!x.XmlParsed && !y.XmlParsed) {
        return string.Equals(x.RawOuterXml, y.RawOuterXml,
    ...
    if (this.Options.IncludeExtendedAttributes) {
      ...
      if (x.ExtendedAttributes != null && y.ExtendedAttributes != null) {
        ...
        for (int i = 0; i < x.ExtendedAttributes.Count(); i++) {
          if (!x.ExtendedAttributes.ElementAt(i).Equals(
                  y.ExtendedAttributes.ElementAt(i))) {
            return false;
          }
        }
      }
    ...
    if (this.Options.IncludeMCAttributes) {
      if (x.MCAttributes == null != (y.MCAttributes == null) ||
          (x.MCAttributes != null && !x.MCAttributes.Equals(y.MCAttributes))) {
        return false;
      }
    }
  ...
  /// <summary>
  /// Calculates a hashcode based on the given <see cref="OpenXmlElement"/>
  /// object.
  /// </summary>
  ...
  private static bool
  PrefixAndQNameEqual(OpenXmlElement x, OpenXmlElement y,
                      OpenXmlElementEqualityOptions options) {
  ...
  private static bool
  MoveNextAndTrackCount(ref OpenXmlElementList.Enumerator e1,
                        ref OpenXmlElementList.Enumerator e2, ref int e1ctr,
                        ref int e2ctr) {
    if (e1.MoveNext()) {
      e1ctr++;
      if (e2.MoveNext()) {
